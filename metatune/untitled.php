	...
	<head>
		...
		<script>
			$(function(){ // document ready
			  if (!!$('.albumArt').offset()) { // make sure ".sticky" element exists
			    var stickyTop = $('.albumArt').offset().top; // returns number 
			    $(window).scroll(function(){ // scroll event
			      var windowTop = $(window).scrollTop(); // returns number 
			      if (stickyTop < windowTop){
			        $('.albumArt').css({ position: 'fixed', top: 0 });
			      }
			      else {
			        $('.albumArt').css('position','static');
			      }
			    });
			  }
			});
